...
DESCRIPTION = (
    "Crisp, upper-class Bengali female late 20s accent with impeccably articulated consonants and perfectly "
...
async def run() -> int:
    ...
    # Accept either HUME_API_KEY (preferred) or HUME_AI_API_KEY for compatibility
    api_key = os.getenv("HUME_API_KEY") or os.getenv("HUME_AI_API_KEY")
    if not api_key:
        raise EnvironmentError(
            "HUME_API_KEY not found. Set HUME_API_KEY in your .env (or HUME_AI_API_KEY as a fallback)."
        )

    # Instantiate client directly (no async context manager)
